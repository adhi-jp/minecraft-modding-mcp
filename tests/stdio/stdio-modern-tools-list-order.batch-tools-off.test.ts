...
// test file/process; this one covers BATCH_TOOLS_OFF=1.
process.env.BATCH_TOOLS_OFF = "1";
// Clear the opposite flag too: an inherited VERIFY_MIXIN_TARGET_OFF=1 would
// silently turn this into the both-off configuration and compare against the
// wrong fixture.
delete process.env.VERIFY_MIXIN_TARGET_OFF;
const root = mkdtempSync(join(tmpdir(), "p3-order-batch-off-"));
...
/**
 * tools/list ordering under BATCH_TOOLS_OFF=1: the legacy era keeps the
...
const GOLDEN = JSON.parse(
  readFileSync(new URL("../fixtures/premigration/tools-list-order.batch-tools-off.json", import.meta.url), "utf8")
...
test("BATCH_TOOLS_OFF=1: modern tools/list is raw name-ascending over the golden config's tool set", async () => {
...
test("BATCH_TOOLS_OFF=1: legacy tools/list keeps the frozen golden registration order", async () => {

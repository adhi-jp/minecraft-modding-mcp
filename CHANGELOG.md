...
### Changed
- `ERR_MAPPING_NOT_APPLIED` now returns a `suggestedCall` of `{ target: { kind: "workspace" }, projectPath, mapping: <detected> }` whenever `projectPath` is supplied and the workspace's compile mapping is non-obfuscated. Cache hits short-circuit; cache misses run a bounded `detectCompileMapping` (build.gradle read + regex) before deciding whether to suggest the workspace retry. Set `WORKSPACE_FALLBACK_LEGACY=1` to restore the previous obfuscated retry shape.

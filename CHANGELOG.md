...
### Fixed
- `validate-access-widener`, `validate-access-transformer`, and `validate-mixin` now correctly remap class references inside JVM method and field descriptors when the validation namespace differs from the jar namespace, eliminating false-negative "method not found" results for entries whose descriptors reference Minecraft classes.
- `validate-access-widener` runtime-aware `scope: "merged"` now auto-detects whether the discovered Loom merged jar uses intermediary or mojang class names instead of assuming intermediary, fixing signature-load failures on mojang-namespace merged jars.

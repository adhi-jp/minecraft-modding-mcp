...
- `pnpm test:manual:stdio-smoke` now falls back to a bash bridge when Node child-process stdio pipes close stdin immediately (for example in Codex), so the manual smoke still validates stdio tool flows instead of always skipping; worker auto-restart remains validated only on native stdio-capable runtimes.

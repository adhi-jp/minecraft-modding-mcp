...
- `analyze-symbol task="lifecycle"` now uses its required `version` as the lifecycle upper bound (`trace-symbol-lifecycle.toVersion`) and limits the high-level helper to a recent 5-version window around that bound. Use `trace-symbol-lifecycle` directly when you need explicit `fromVersion` / `toVersion` control or a wider history.
...
- `find-mapping` also restores the main named-namespace paths (`mojang <-> intermediary`, `mojang -> yarn`) when upstream Tiny metadata still uses the runtime header name `official`.
...
- `trace-symbol-lifecycle` rejects class-like `symbol` inputs such as `net.minecraft.world.item.Item`; pass `Class.method` and keep exact overload matching in the separate `descriptor` field.
...
- `trace-symbol-lifecycle`, `check-symbol-exists`, and `find-mapping` now skip intermediary/yarn Tiny graph loading when a request only needs the Mojang/obfuscated namespace pair, reducing cold-path lifecycle latency without changing the public response shape.
- `inspect-minecraft task="class-overview"` / `search` / `class-source` / `class-members` still need artifact context for plain `subject.kind="search" | "class" | "file"` inputs. If you do not already have an artifact, use `subject.kind="workspace"` or add `subject.artifact`. Invalid combinations now return a retryable `suggestedCall` that preserves the requested task when artifact context is the only missing input.
- When a workspace resolves only partial source coverage and misses `net.minecraft`, `inspect-minecraft task="class-overview"` and class-like workspace `task="search"` now confirm vanilla classes through binary-backed symbol lookup, and workspace `task="list-files"` returns `summary.status="partial"` with follow-up guidance.

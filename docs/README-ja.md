...
### v3 エントリーツール (v3 Entry Tools)

v3 で最初に使うことを想定した高レベルツールです。どのツールも公開 `{ result?, error?, meta }` エンベロープを維持し、常に `result.summary` を返します。`detail: "summary" | "standard" | "full"` と opt-in の `include[]` を共有し、変更系エントリーツールは `executionMode: "preview" | "apply"` で preview/apply を明示します。

| ツール | 機能 | 主な入力 | 主な出力 |
| --- | --- | --- | --- |
| `inspect-minecraft` | version / artifact / class / file / search / workspace から最適な Minecraft 調査フローへルーティング | `task?`, `subject?`, `detail?`, `include?`, `limit?`, `cursor?`, `includeSnapshots?` | `result.summary`, `subject`, `artifact?`, `class?`, `search?`, `file?`, `files?` |
| `analyze-symbol` | シンボル存在確認、名前空間マッピング、ライフサイクル追跡、workspace symbol 解析、API overview を 1 つに統合 | `task`, `subject`, `version?`, `sourceMapping?`, `targetMapping?`, `projectPath?`, `classNameMapping?`, `detail?`, `include?` | `result.summary`, `match?`, `candidates?`, `ambiguity?`, `matrix?`, `workspace?` |
| `compare-minecraft` | version pair 比較、class signature diff、registry diff、migration-oriented overview | `task?`, `subject`, `detail?`, `include?`, `sourcePriority?`, `maxClassResults?`, `maxEntriesPerRegistry?`, `includeFullDiff?` | `result.summary`, `comparison`, `classes?`, `classDiff?`, `registry?`, `migration?` |
| `analyze-mod` | Mod summary を cheap path にした metadata-first entry tool。decompile/search/class-source/remap へ拡張 | `task`, `subject`, `query?`, `searchType?`, `targetMapping?`, `outputJar?`, `executionMode?`, `detail?`, `include?` | `result.summary`, `metadata?`, `decompile?`, `hits?`, `source?`, `operation?` |
| `validate-project` | workspace summary と direct Mixin / Access Widener validation をまとめた project-oriented entry tool | `task`, `subject`, `version?`, `mapping?`, `detail?`, `include?`, `sourceRoots?`, `configPaths?`, validator controls | `result.summary`, `project`, `workspace?`, `issues?`, `recovery?` |
...
`compare-minecraft` は class-diff 系フローで `sourcePriority` をそのまま受け取り、`diff-class-signatures` と同じ優先順指定を使えます。
`inspect-minecraft` は `task="class-overview" | "class-source" | "class-members"` でも `subject.kind="workspace"` + `focus.kind="class"` を受け付け、focus 側に `artifact` が無い場合は workspace から解決した version を使って artifact context を補完します。
`compare-minecraft` の summary-mode version 比較は、class / registry のサンプルが切り詰められたときに `meta.truncated` と retry 用 `nextActions` を返し、`include=["classes"]` / `["registry"]` で片側だけ展開した場合も残りの summary 側切り詰めを明示します。
`registry-diff` で detailed registry data を片側しか読めない場合でも、ツール全体を失敗させず `summary.status="partial"` と recovery 向け `nextActions` を返します。

`manage-cache` の list pagination は `meta.pagination.nextCursor` を返します。
`selector.olderThan` は `P30D` のような ISO-8601 duration を受け付け、`selector.status` は `healthy | partial | stale | orphaned | corrupt | in_use` を使います。`jarPath` / `projectPath` の比較前には WSL / Windows のパス表現を正規化します。

low-level expert tool を正確に使い分けたい場合は、以下の retained expert tool を直接呼んでも構いません。
...
移行メモ:
- version / artifact / class / file / search 系の調査は、まず `inspect-minecraft` を使い、その後に必要なら `list-versions`、`resolve-artifact`、`get-class-source`、`get-class-members`、`search-class-source`、`get-artifact-file`、`list-artifact-files` へ降りてください。
- symbol mapping / existence / lifecycle / workspace 系は、まず `analyze-symbol` を使い、その後に `find-mapping`、`resolve-method-mapping-exact`、`check-symbol-exists`、`trace-symbol-lifecycle`、`resolve-workspace-symbol`、`get-class-api-matrix` を直接使ってください。
- version pair / class diff / registry diff / migration summary は、まず `compare-minecraft` を使い、その後に `compare-versions`、`diff-class-signatures`、`get-registry-data` を直接使ってください。
- mod metadata-first inspection と safe remap preview/apply は、まず `analyze-mod` を使い、その後に `analyze-mod-jar`、`decompile-mod-jar`、`get-mod-class-source`、`search-mod-source`、`remap-mod-jar` を直接使ってください。
- workspace summary と direct Mixin / Access Widener validation は、まず `validate-project` を使い、その後に `validate-mixin` または `validate-access-widener` を直接使ってください。
- cache inventory と安全な cleanup は、まず `manage-cache` を使い、`executionMode="preview"` で影響を確認してから `executionMode="apply"` を使ってください。

...
**35 ツール**（6 エントリ + 29 エキスパート） | **7 リソース** | **4 名前空間マッピング** | **SQLite キャッシュ**
...
| `compare-minecraft` | version pair 比較、class signature diff、registry diff、migration-oriented overview | `task?`, `subject`, `detail?`, `include?`, `sourcePriority?`, `maxClassResults?`, `maxEntriesPerRegistry?`, `includeFullDiff?`, `limit?` | `result.summary`, `comparison`, `classes?`, `classDiff?`, `registry?`, `migration?` |
...
`get-class-members` は、classfile に不正な field/method descriptor が含まれている場合、破損したバイトコードから不正な Java シグネチャテキストを出力する代わりに `ERR_INVALID_INPUT` を返します。
method の return 位置以外での不正な `void` 使用は descriptor 固有のエラーで拒否されるため、無効なメソッド引数は field descriptor エラーではなく method descriptor エラーとして報告されます。

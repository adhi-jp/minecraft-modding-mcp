...
- `analyze-symbol task="lifecycle"` は、必須の `version` をライフサイクル走査の上限 (`trace-symbol-lifecycle.toVersion`) として使い、高レベル helper としてはその近傍 5 バージョン分に走査を絞ります。`fromVersion` / `toVersion` を明示制御したい場合や、より広い履歴が必要な場合は low-level の `trace-symbol-lifecycle` を直接使ってください。
- `get-class-api-matrix` は、複数名前空間が利用可能でも明示した `classNameMapping` を基準に行列を組み、暗黙に `obfuscated` へ戻りません。
- `find-mapping` は `sourceMapping="obfuscated"` のとき、`dhl` のような短い難読化クラス ID も受け付けます。その他のクラスマッピング検索は引き続き完全修飾名を前提とします。
- `find-mapping` は、upstream Tiny メタデータが runtime namespace を `official` と書いている場合でも、主要な named path（`mojang <-> intermediary`, `mojang -> yarn`）を解決します。
- `trace-symbol-lifecycle` は厳密照合では引き続き別フィールドの `descriptor` を推奨しますが、`symbol` に誤ってインライン署名が含まれていても `Class.method` 部分を先に切り出して解析します。
- `trace-symbol-lifecycle` は `net.minecraft.world.item.Item` のような class-like な `symbol` を受け付けず、`ERR_INVALID_INPUT` を返します。`Class.method` を渡し、厳密な overload 指定は別フィールドの `descriptor` を使ってください。
- `trace-symbol-lifecycle` のバージョンごとの bytecode 確認は、レスポンス契約を変えずに bounded parallelism で実行されるようになり、広い範囲の走査でも停止しにくくなりました。
- `trace-symbol-lifecycle` / `check-symbol-exists` / `find-mapping` は、Mojang と obfuscated の namespace だけを使う request では intermediary/yarn の Tiny graph を読まないようになり、cold path の lifecycle 系レイテンシを抑えます。
- plain な `subject.kind="search" | "class" | "file"` で `inspect-minecraft task="class-overview" | "search" | "class-source" | "class-members"` を使う場合は、引き続き artifact context が必要です。artifact がまだ無い場合は `subject.kind="workspace"` を使うか、`subject.artifact` を追加してください。artifact context だけが不足している場合、retry 用の `suggestedCall` は元の task を保ったまま返ります。
- workspace が partial source しか解決できず `net.minecraft` を欠く場合でも、`inspect-minecraft task="class-overview"` と class-like な workspace `task="search"` は binary-backed symbol lookup で vanilla class を確認できます。workspace `task="list-files"` は `summary.status="partial"` と follow-up guidance を返します。
- `analyze-mod` と `validate-project` は引き続き structured な `subject` と canonical な `include` を要求しますが、古い string-subject / domain-include payload には `ERR_INVALID_INPUT` と再試行しやすい `suggestedCall` を返します。
- `validate-mixin` と `validate-project task="project-summary"` は、Mixin クラス定義を持たない空 config を `ERR_INVALID_INPUT` ではなく、検出結果 0 件の warning として扱います。
...
パス系の上書き設定では、空文字列と文字列 `undefined` / `null` を未設定として扱います。クライアント側の誤シリアライズで `./undefined` や `./null` のキャッシュルート、壊れた JAR パスが作られないようにするためです。

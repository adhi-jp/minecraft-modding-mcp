...
`resolve-artifact`、`get-class-source`、`get-class-members`、`search-class-source`、`get-artifact-file`、`list-artifact-files` は `artifactContents` を返し、アーティファクトが `source-jar` 由来か `decompiled-binary` 由来か、resources がインデックスされているか（現状は `resourcesIncluded=false`）、source coverage が `full` か `partial` かを確認できます。

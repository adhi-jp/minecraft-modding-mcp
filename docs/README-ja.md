...
- `analyze-symbol task="lifecycle"` は、必須の `version` をライフサイクル走査の上限 (`trace-symbol-lifecycle.toVersion`) として使います。`fromVersion` / `toVersion` を明示制御したい場合は low-level の `trace-symbol-lifecycle` を直接使ってください。

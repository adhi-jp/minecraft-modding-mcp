...
v3 で最初に使うことを想定した高レベルツールです。どのツールも公開 `{ result?, error?, meta }` エンベロープを維持し、常に `result.summary` を返します。`summary.subject` と `summary.nextActions` は follow-up tool call を機械的に組めるように整形されています。`detail: "summary" | "standard" | "full"` と opt-in の `include[]` を共有し、変更系エントリーツールは `executionMode: "preview" | "apply"` で preview/apply を明示します。
...
`compare-minecraft` は class-diff 系フローで `subject.kind="class".sourcePriority` をそのまま受け取り、`diff-class-signatures` と同じ優先順指定を使えます。
...
`search-class-source` の `queryMode` は検索戦略を制御します。`auto`（デフォルト）と `token` は `foo.bar` / `foo_bar` / `foo$bar` のような区切り文字クエリも含めてインデックス経路に留まり、`literal` は明示的な部分文字列スキャン用です。

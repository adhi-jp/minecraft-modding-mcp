...
| `resolve-artifact` | `version` / `jar` / `coordinate` からソースアーティファクトを解決 | `targetKind`, `targetValue`, `mapping?`, `sourcePriority?`, `allowDecompile?`, `projectPath?`, `scope?`, `preferProjectVersion?`, `strictVersion?` | `artifactId`, `origin`, `mappingApplied`, `qualityFlags[]`, `adjacentSourceCandidates?`, `sampleEntries?`, `warnings[]` |
| `find-class` | アーティファクト内で簡易名または FQCN からクラスを解決 | `className`, `artifactId`, `limit?` | `matches[]`, `total`, `warnings[]` |
| `get-class-source` | `artifactId` またはオンデマンド解決でクラスソースを取得（デフォルト `mode=metadata`） | `className`, `mode?`, `artifactId?`, `targetKind?`, `targetValue?`, `projectPath?`, `scope?`, `preferProjectVersion?`, `strictVersion?`, `startLine?`, `endLine?`, `maxLines?`, `maxChars?`, `outputFile?` | `mode`, `sourceText`, `returnedRange`, `truncated`, `charsTruncated?`, `outputFile?`, `artifactId`, マッピング/来歴メタデータ |
| `get-class-members` | バイトコードからクラスのフィールド/メソッド/コンストラクタを取得 | `className`, `artifactId?`, `targetKind?`, `targetValue?`, `mapping?`, `access?`, `includeInherited?`, `maxMembers?`, `strictVersion?` | `members.{constructors,fields,methods}`, `counts`, `truncated`, `context`, `warnings[]` |
| `search-class-source` | インデックス化されたクラスソースからシンボル/テキスト/パスを検索 | `artifactId`, `query`, `intent?`, `match?`, `packagePrefix?`, `fileGlob?`, `symbolKind?`, `snippetLines?`, `includeDefinition?`, `includeOneHop?`, `queryMode?`, `limit?`, `cursor?` | `hits[]`, `relations?`, `nextCursor?`, `totalApprox`, `mappingApplied` |
...
| ツール | 機能 | 主な入力 | 主な出力 |
| --- | --- | --- | --- |
| `find-mapping` | クラス/フィールド/メソッドのシンボルに対するマッピング候補を名前空間間で検索 | `version`, `kind`, `name`, `owner?`, `descriptor?`, `sourceMapping`, `targetMapping`, `sourcePriority?`, `disambiguation?` | `querySymbol`, `mappingContext`, `resolved`, `status`, `resolvedSymbol?`, `candidates[]`, `ambiguityReasons?`, `provenance?`, `meta.warnings[]` |
...
| `get-class-api-matrix` | 1つのクラス API をマッピングマトリクスとして表示（`official/mojang/intermediary/yarn`） | `version`, `className`, `classNameMapping`, `includeKinds?`, `sourcePriority?` | `classIdentity`, `rows[]`, `ambiguousRowCount?`, `meta.warnings[]` |
...
| `check-symbol-exists` | クラス/フィールド/メソッドの厳密な存在確認 | `version`, `kind`, `name`, `owner?`, `descriptor?`, `sourceMapping`, `sourcePriority?`, `nameMode?`, `signatureMode?` | `querySymbol`, `mappingContext`, `resolved`, `status`, `resolvedSymbol?`, `candidates[]`, `meta.warnings[]` |
...
`validate-mixin` は `source`、`sourcePath`、`sourcePaths`、`mixinConfigPath` のいずれか1つが必要です。`sourcePath`/`sourcePaths[]` はファイル読み取り前にホスト/WSL パス形式に正規化されます。`mixinConfigPath` は Mixin 設定 JSON を読み取り、バッチバリデーション用のソースファイルを自動検出します（`sourceRoots[]` または `sourceRoot` でルックアップルートを上書き可能。未指定時は `src/main/java`、`src/client/java`、`common/src/{main,client}/java`、`fabric/src/{main,client}/java`、`neoforge/src/{main,client}/java`、`forge/src/{main,client}/java`、`quilt/src/{main,client}/java` から自動検出）。
`validate-mixin` の単一ファイルレスポンスにはマッピングフォールバック時に `provenance.resolutionNotes?` が含まれます。
`validate-mixin` は `@Invoker` ターゲットをメソッドのみ、`@Accessor` ターゲットをフィールドのみに対してバリデーションします。
`validate-mixin` のパーサーは `.class` リテラルターゲットと `targets = "..."` / `targets = {"a", "b"}` 文字列形式の両方をサポートします。
`validate-mixin` のパーサーは `@Shadow`/`@Accessor` と宣言間の複数行アノテーションを処理し、宣言行からインラインアノテーションを除去します。
`validate-mixin` は `target-mapping-failed`（warning、uncertain）と `target-not-found`（error）をクラスマッピング失敗時に区別します。
`validate-mixin` の issues と `structuredWarnings` には `category`（`mapping`、`configuration`、`validation`、`resolution`、`parse`）が含まれ、セットアップ/ツーリング/パーサーの制限と実際のバリデーションエラーを区別します。
`validate-mixin` は `minSeverity`、`hideUncertain`、`warningCategoryFilter` によるフィルタリングをサポートし、`treatInfoAsWarning=false` で `structuredWarnings` の info レベルエントリを抑制できます。
`validate-mixin` の単一ファイルレスポンスには各メンバーの解決状態（`resolved` または `not-found`）を追跡する `resolvedMembers?` が含まれます。
`validate-mixin` の `explain=true` は各 issue に `explanation` と `suggestedCall`（tool + params）を付加し、エージェント駆動のリカバリを支援します。
`validate-mixin` のバッチ `summary` は `processingErrors`（例外数）、`totalValidationErrors`、`totalValidationWarnings` を使用します。
`resolve-artifact` の `targetKind=version` は `mapping=mojang` の場合のみ `projectPath` から Loom キャッシュ検出を使用します。マッピング失敗時は `searchedPaths`、`candidateArtifacts`、`recommendedCommand` をエラー詳細に含みます。
`resolve-artifact` は `scope`（`vanilla`/`merged`/`loader`）と `preferProjectVersion=true`（`targetKind=version` 時に `gradle.properties` から `minecraft_version`/`mc_version`/`minecraftVersion` で `targetValue` を上書き）をサポートします。
`resolve-artifact` はソース JAR 解決時にのみ `sampleEntries` を含みます。デコンパイル専用パスでは未設定です。
`find-class` はタイプシンボル（`class`/`interface`/`enum`/`record`）のみを返します。FQCN ルックアップは正確な FQCN/ファイルパスでフィルタリングされ、同名の多数のクラスによる偽陰性を回避します。
...
`search-class-source` の `queryMode` はテキスト検索戦略を制御します: `auto`（デフォルト）はインデックストークン検索＋セパレータクエリ時のリテラルフォールバック、`token` はインデックストークン検索のみ、`literal` は部分文字列スキャンのみです。
`search-class-source` の `match=regex` は `query.length <= 200` と厳格な結果上限 `100` を適用します。
`search-class-source` の `fileGlob` は `*` / `**` / `?` をサポートし、`net/minecraft/**/*.java` のような再帰パターンを使用できます。
`get-class-source` のフォールバック照合はパッケージ互換性を強制し、他パッケージの同名クラスしか存在しない場合は `ERR_CLASS_NOT_FOUND` を返します。
`get-class-source` の mode はデフォルト `metadata`（シンボルアウトラインのみ）です。`mode=snippet` は行範囲/max 未指定時に `maxLines=200` を自動設定します。`mode=full` はソース全体を返します。`outputFile` は選択テキストをファイルに書き出し、`outputFile` にパスを返します。
デコンパイルフォールバック（`resolve-artifact`/`get-class-source`）は Vineflower フラグを位置引数の前に配置し、有効な JAR での偽 `ERR_DECOMPILER_FAILED` を防ぎます。
...
`targetKind=coordinate` でクラシファイア付き（`group:artifact:version:classifier`）の場合、ローカル Maven ソース検索は `<artifact>-<version>-<classifier>-sources.jar` を最初に、次に `<artifact>-<version>-sources.jar` を確認します。
`get-artifact-file` のバイト切り捨ては UTF-8 文字境界を保持し、`maxBytes` がマルチバイトテキストを切断した際の置換文字（`�`）の破損を防ぎます。
Mod ツールの `jarPath` 入力は、存在確認・キャッシュキー・処理の前に正規のローカル `.jar` ファイルパスに正規化されます。
`search-mod-source` は `query.length <= 200` と `limit <= 200` を適用します。
`search-mod-source` はソース専用 JAR を検出し、デコンパイルなしで `.java` エントリを直接検索します。
`get-mod-class-source` は `maxLines`、`maxChars`、`outputFile` をサポートし、`get-class-source` と同様の切り捨て動作を行います。`outputFile` 設定時は書き出されたファイルに切り捨てが反映されます。
`find-mapping` は `status=ambiguous` 時に候補を一意に解決できなかった理由を説明する `ambiguityReasons` を返します。
`get-class-api-matrix` は1行以上のあいまいフォールバックが適用された場合に `ambiguousRowCount` を返します。
`check-symbol-exists` はデフォルトで厳密な FQCN クラス入力を要求します。`nameMode=auto` で短いクラス名を許可できます（あいまいな一致は `status=ambiguous` を返します）。
`check-symbol-exists` は `signatureMode=name-only` でディスクリプタなしの owner+name マッチングをサポートします。単一一致は `resolved`、複数オーバーロードは全候補付きの `ambiguous` を返します。
`check-symbol-exists` は常に入力形状を最初にバリデーションし、マッピングデータが利用不可でも無効なシンボル組み合わせには `ERR_INVALID_INPUT` を返します。
